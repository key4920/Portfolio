...
##### 데이터 로딩
raw <- read.csv("./data_merged/data_anal2.csv", header=T, sep=",",
...
names <- c('del_period_deadline_yn', # 배송변수 중
           ...
           'customer_state', 'customer_zip_code_prefix','seller_state','seller_zip_code_prefix') # 위치변수 중
#?factor
for (i in names) {
...
#- 해당 방법은 선형회귀 모형을 응용한 의사결정나무(decision tree) 기법이다.
#- withe box 기법으로, 결과를 설명해줌. 그래서 기본적으로 많이 쓰는 기법임. 
...
f1 <- pn_review_score ~ del_period + del_period_psy + del_period_deadline_yn + 
  ...
  payment_voucher_yn + order_freight_value + freight_value_proportion + order_product_value + 
  order_time_6 + order_day + order_week_day_end + 
  review_ans_time_6 + review_ans_day + review_ans_w_d_e + order_day_time_6 +
  installments_yn + sim_installments_mean +
  review_ans_period + review_comment_yn +
  seller_response_time +
  customer_state + customer_zip_code_prefix + seller_state + seller_zip_code_prefix

set.seed(100)
fit1 <- ctree(f1, data = raw_factored, controls = ctree_control(maxdepth = 4))
pdf('./images_tree/2_tree_모든변수.pdf', width = 30, height = 8)
...
pdf('./images_tree/2_tree_모든변수_train.pdf', width = 30, height = 8)
plot(fit1, gp = gpar(fontsize=1))
dev.off()
...
pred
confusionMatrix(pred3, test$pn_review_score)

#################################################################### 2_모든변수-reviewAnswer

f2 <- pn_review_score ~ del_period + del_period_psy + del_period_deadline_yn + 
  product_description_length + product_photos_qty + 
  payment_voucher_yn + order_freight_value + freight_value_proportion + order_product_value + 
  order_time_6 + order_day + order_week_day_end + 
  + order_day_time_6 +
...
set.seed(100)
fit2 <- ctree(f2, data = train, controls = ctree_control(maxdepth = 4))
pdf('./images_tree/2_tree_모든변수-reviewAnswer_train.pdf', width = 30, height = 8)
plot(fit2, gp = gpar(fontsize=1))
dev.off()
...
#################################################################### 2_모든변수-reviewAnswer-prefix-state+distance

f3 <- pn_review_score ~ del_period + del_period_psy + del_period_deadline_yn + 
  product_description_length + product_photos_qty + 
  payment_voucher_yn + order_freight_value + freight_value_proportion + order_product_value + 
  order_time_6 + order_day + order_week_day_end + 
  + order_day_time_6 +
...
set.seed(100)
fit3 <- ctree(f3, data = train, controls = ctree_control(maxdepth = 4))
pdf('./images_tree/2_tree_모든변수-reviewAnswer-prefix-state+distance_train.pdf', width = 30, height = 8)
plot(fit3, gp = gpar(fontsize=1))
dev.off()
...
##### 컬럼명 변경
names(raw) <- c("pn_review_score","del_period_deadline_yn","del_period",
                  "distance","freight_value_proportion","order_product_value_range",
                  "del_period_psy","order_day","installments_yn")
str(raw)
...
names <- c('del_period_deadline_yn','order_product_value_range','order_day','installments_yn')
...
set.seed(100)
fit1 <- ctree(f1, data = train, controls = ctree_control(maxdepth = 4))
png('./images_tree/8_tree.png', width = 2000, height = 800)
#pdf('./images_tree/8_tree.pdf', width = 30, height = 8)
plot(fit1, gp = gpar(fontsize=1))
dev.off()
